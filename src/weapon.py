...
class Weapon(pygame.sprite.Sprite):
    def __init__(self, player, enemies=None, target_pos=None):
        super().__init__()
        ...
        direction = None
        # 수동 타겟 위치가 있으면 그 방향으로 설정
        if target_pos:
            direction = pygame.math.Vector2(target_pos) - pygame.math.Vector2(self.player.rect.center)
        # 자동 모드이거나, 수동 모드인데 타겟이 없으면 가장 가까운 적을 찾음
        elif enemies:
            target = self.get_closest_enemy(enemies)
            if target:
                direction = pygame.math.Vector2(target.rect.center) - pygame.math.Vector2(self.player.rect.center)
